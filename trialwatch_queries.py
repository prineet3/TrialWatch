...
Notes:
- Built on the loaded MongoDB dashboard collections.
- Dollar amounts use `funding_deduped`, adverse events use `ae_count_deduped`.
- Column names are normalized to snake_case in the loader.
"""
    ...
    pipeline = [
        ...
        {"$unwind": "$trial"},
        {
            "$group": {
                ...
                "late_count": {
                    "$sum": {
                        "$cond": [{"$eq": ["$compliance_status", "LATE"]}, 1, 0]
                    }
                },
                "missing_count": {
                    "$sum": {
                        "$cond": [{"$eq": ["$compliance_status", "MISSING"]}, 1, 0]
                    }
                },
...
def get_trial_detail(db, nct_id):
    """Full unified record for a single trial."""
    pipeline = [
        {"$match": {"_id": nct_id}},
        {
            "$lookup": {
